...
CAN_RATE = {
    "5KBPS"   : [0x31, 0XBF, 0x06],
    "10KBPS"  : [0x31, 0x93, 0x03],
    "20KBPS"  : [0x31, 0x81, 0x01],
    "50KBPS"  : [0x0B, 0x92, 0x02],
    "100KBPS" : [0x05, 0x92, 0x02],
    "125KBPS" : [0x01, 0xB7, 0x07],
    "250KBPS" : [0x00, 0xB7, 0x07],
    "500KBPS" : [0x00, 0x8B, 0x04],
    "1000KBPS": [0x00, 0x81, 0x01],
}
...
class MCP2515():
    def __init__(self):
        ...
        self.GPIO = GPIO
        self.GPIO.setup(SPI0_CS,GPIO.OUT)
        self.GPIO.setup(INT,GPIO.IN)
    ...
    def ReadByte(self, addr):
        self.Pin_write(SPI0_CS, 0)
        
        self.spi.writebytes(bytearray([CAN_READ]))
        self.spi.writebytes(bytearray([addr]))
        res = self.spi.readbytes(1)
        
        self.Pin_write(SPI0_CS, 1)
    ...
    def WriteBytes(self, addr, data):
        ...
        self.spi.writebytes(bytearray([CAN_WRITE]))
        self.spi.writebytes(bytearray([addr]))
    ...
    def Reset(self):
        self.Pin_write(SPI0_CS, 0)
        self.spi.writebytes(bytearray([CAN_RESET])) #Reset 0XC0
        self.Pin_write(SPI0_CS, 1)
...
if __name__ == '__main__':
    ...
    can = MCP2515()
    can.Init("1000KBPS")   # The default baud rate is 1M
    id = 0x0 #max 7ff 
    data = [0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0xaa]
    dlc = 8
    hex_list = [hex(x) for x in data]
    
    print("Start sending CAN data")
    print("The data sent by CAN is : ", hex_list)
